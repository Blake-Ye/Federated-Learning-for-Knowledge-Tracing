...
# Ensure the device is GPU (if available), otherwise use CPU
...
class FlowerClient(fl.client.NumPyClient):
    def __init__(self, train_loader, test_loader, model) -> None:
        ...
        self.model = model.to(device)  # Ensure the model is on GPU
        self.opt = Adam(self.model.parameters(), lr=learning_rate)  # Use Adam optimizer

    def set_parameters(self, parameters):
        """Update model parameters"""
        params_dict = zip(self.model.state_dict().keys(), parameters)
        state_dict = OrderedDict({k: torch.Tensor(v).to(device) for k, v in params_dict})  # Move to GPU
...
def generate_client_fn(train_loader, test_loader, device="cpu"):
    """Generate client function ensuring each client independently handles data partitioning and device allocation"""
    ...
    def client_fn(context: fl.common.Context):
        ...
        train_end_idx = (partition_id + 1) * train_partition_size if partition_id != num_partitions - 1 else len(train_dataset)
